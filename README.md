...
20. 매칭된 사용자에게 인앱 알림함 notification을 저장하고 push delivery 상태와 읽음 상태를 관리한다. Hana 세무 상태 sync가 `RECAPTURE_RISK`를 반환하면 해당 tax refund case subject로 사후 환수 리스크 notification을 한 번 저장한다. 현재 provider는 외부 발송 없는 `LOCAL_NOOP_PUSH`이며, 실패/미발송 notification은 retry worker가 설정된 batch와 max attempt 기준으로 재시도한다. FCM/APNS/web push provider는 다음 단계에서 연결한다.
