...
- `/api/v1/accounts/**`는 Spring Security bearer filter로 보호하며, token의 `accountId`와 path의 `accountId`가 일치해야 한다. trade holding/ledger는 Flyway/JDBC로 영속화하며, watchlist, alert, notification, tax 저장소 영속화는 별도 단계에서 추가한다.
