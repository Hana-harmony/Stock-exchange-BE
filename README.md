...
- Notification: `LOCAL_NOOP_PUSH` provider로 alert와 tax recapture risk delivery 상태 기록 구현, FCM/APNS/web push provider routing과 미설정 provider `SKIPPED` 기록 구현, 실패/미발송 notification retry worker 구현, 실제 외부 push 자격증명·디바이스 토큰 연동 예정
- 구독 topic:
...
20. 매칭된 사용자에게 인앱 알림함 notification을 저장하고 push delivery 상태와 읽음 상태를 관리한다. Hana 세무 상태 sync가 `RECAPTURE_RISK`를 반환하면 해당 tax refund case subject로 사후 환수 리스크 notification을 한 번 저장한다. 기본 provider는 외부 발송 없는 `LOCAL_NOOP_PUSH`이며, `EXCHANGE_NOTIFICATION_PUSH_ENABLED_PROVIDERS`로 FCM/APNS/web push routing을 켤 수 있다. 외부 provider는 자격증명과 디바이스 토큰이 없으면 `SKIPPED`로 기록하고, 실패/미발송 notification은 retry worker가 설정된 batch와 max attempt 기준으로 재시도한다.
