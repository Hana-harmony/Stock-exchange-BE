...
- `notification`: FCM/APNS/web push provider routing, FCM encrypted token vault와 HTTP v1 send client, 미설정 provider SKIPPED 상태, provider별 retry worker 연동
- Planned `notification`: 실제 APNS/web push 자격증명과 외부 provider 실발송
...
- notification은 provider 추상화와 delivery 상태, 계좌별 device token 등록 상태를 보관한다. 기본 provider는 외부 발송 없는 `LOCAL_NOOP_PUSH`이며, `EXCHANGE_NOTIFICATION_PUSH_ENABLED_PROVIDERS`로 FCM/APNS/web push routing을 켤 수 있다. 현지 거래소 앱은 iOS/Android/web device token을 등록·조회·비활성화할 수 있고, API 응답은 원문 token 대신 hash와 masked token만 노출한다. FCM provider는 AES-GCM encrypted token vault와 HTTP v1 send client를 사용하며, token vault key 또는 FCM project/access token credential이 없으면 `SKIPPED`로 기록한다. 실패/미발송 notification은 `EXCHANGE_NOTIFICATION_PUSH_WORKER_ENABLED=true`일 때 retry worker가 batch size와 max attempt 설정 기준으로 재전송한다. 실제 APNS/web push 자격증명과 provider 실발송은 통합 단계다.
...
- FCM HTTP v1 provider 실발송 client는 구현되어 있으며, APNS/web push provider 실발송과 외부 object storage provider는 통합 단계다. 세무 파일은 로컬 object storage adapter로 저장하며, Hana REST retry/backoff, notification provider routing/retry worker, audit retention worker는 기본 설정으로 구현되어 통합 환경에서 활성화할 수 있다.
