...
- Planned `auth`: refresh token/session hardening
...
- 로그인 API는 저장된 PBKDF2 hash를 검증하고 HMAC 기반 local JWT를 발급한다. token verify API는 FE session context가 사용할 userId, username, accountId, expiry를 반환한다.
...
- 영속 DB schema, refresh token, push worker, 웹 푸시는 미구현이다.
