...
- 로그인 API는 PBKDF2 password verify, local JWT 발급/검증, refresh token rotation/logout 계약을 제공하고, `/api/v1/accounts/**`는 Spring Security bearer filter로 보호한다. refresh session 영속화는 별도 DB 단계로 확장한다.
