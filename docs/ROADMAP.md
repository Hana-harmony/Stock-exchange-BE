...
- 아이디/비밀번호 회원가입, 로그인, mock USD 계좌 생성: 인메모리 API, local JWT 발급/검증, Spring Security bearer filter 구현 완료, 영속화와 refresh token 예정
...
- 알림함 API: 인메모리 조회/읽음 처리와 push delivery 상태 구현 완료, 영속화 예정
