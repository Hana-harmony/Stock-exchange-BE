...
@Component
public class HanaAlertStreamClient implements SmartLifecycle {
	...
	private final HanaAlertStreamMessageHandler messageHandler;
	...
	public HanaAlertStreamClient(
			...
			HanaAlertStreamMessageHandler messageHandler) {
	...
	@Override
	public void start() {
		if (running || !properties.stream().alertEnabled()) {
			return;
		}
		running = true;
		drainTask = taskScheduler.scheduleWithFixedDelay(
				messageHandler::drainBufferedEvents,
	...
	private void connect() {
		...
		webSocketClient.execute(new AlertWebSocketHandler(), headers, streamUri())
	...
	private URI streamUri() {
		...
		String path = properties.stream().alertPath().startsWith("/")
				? properties.stream().alertPath()
				: "/" + properties.stream().alertPath();
		return URI.create(scheme + withoutScheme + path);
	}

	private void sendReplayRequest(WebSocketSession connectedSession) {
		if (!properties.stream().alertReplayEnabled()) {
			return;
		}
	...
	private class AlertWebSocketHandler extends TextWebSocketHandler {
		...
		public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
			HanaAlertStreamClient.this.session.compareAndSet(session, null);
			scheduleReconnect();
		}
	}
}
